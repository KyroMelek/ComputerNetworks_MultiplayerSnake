...
public class Snake : MonoBehaviour
{
    ...
    public void updateHeadAndBodyPositions(List<Vector2> positions)
    {
        if (!localPlayer)
        {
            while (bodySegmentContainer.transform.childCount < positions.Count - 1)
                addBodySegment(Vector2.zero);

            headSegment.transform.position = positions[0];

            for (int i = 1; i < positions.Count; i++)
                bodySegmentObjects[i - 1].transform.position = positions[i];
        }
    }
}
